...
class HttpUtil {
    ...
    static class PartLexer {
        ...
        private String consumeAlpha() {
            while (true) {
                char charAt = this.src.charAt(this.f7919p);
                if (this.f7919p >= this.src.length() || ((charAt < 'a' || charAt > 'z') && (charAt < 'A' || charAt > 'Z'))) {
                    String substring = this.src.substring(this.last, this.f7919p);
                    this.last = this.f7919p;
                } else {
                    this.f7919p++;
                }
            }
            String substring2 = this.src.substring(this.last, this.f7919p);
            this.last = this.f7919p;
            return substring2;
        }

        private boolean consumeIf(char c) {
            if (this.f7919p >= this.src.length() || this.src.charAt(this.f7919p) != c) {
                return false;
            }
            this.f7919p++;
            return true;
        ...
        private String consumeUntil(char c) {
            while (this.f7919p < this.src.length() && this.src.charAt(this.f7919p) != c) {
                this.f7919p++;
            }
            String substring = this.src.substring(this.last, this.f7919p);
            this.last = this.f7919p;
        ...
        private void skipWhiteSpace() {
            while (this.f7919p < this.src.length() && this.src.charAt(this.f7919p) < '!') {
                this.f7919p++;
            }
            this.last = this.f7919p;
        }
        ...
        public Map<String, String> Parse() {
            ...
            while (this.f7919p < this.src.length()) {
                skipWhiteSpace();
