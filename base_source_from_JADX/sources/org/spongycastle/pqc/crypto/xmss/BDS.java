        ...
        if (oTSHashAddress != null) {
            ...
            for (int i = 0; i < (1 << this.treeHeight); i++) {
                ...
                while (!this.stack.isEmpty() && this.stack.peek().getHeight() == lTree.getHeight()) {
                    ...
                    if (floor == 3 && lTree.getHeight() < this.treeHeight - this.f8224k) {
                        this.treeHashInstances.get(lTree.getHeight()).setNode(lTree.clone());
                    }
                    if (floor >= 3 && (floor & 1) == 1 && lTree.getHeight() >= this.treeHeight - this.f8224k && lTree.getHeight() <= this.treeHeight - 2) {
        ...
        if (oTSHashAddress == null) {
            ...
            if (calculateTau == 0) {
                ...
                for (int i2 = 0; i2 < calculateTau; i2++) {
                    if (i2 < this.treeHeight - this.f8224k) {
                        list = this.authenticationPath;
                        obj = this.treeHashInstances.get(i2).getTailNode();
                    } else {
                        list = this.authenticationPath;
                        obj = this.retain.get(Integer.valueOf(i2)).removeFirst();
                    }
                    list.set(i2, obj);
                }
                int min = Math.min(calculateTau, this.treeHeight - this.f8224k);
            ...
            for (int i5 = 0; i5 < ((this.treeHeight - this.f8224k) >> 1); i5++) {
