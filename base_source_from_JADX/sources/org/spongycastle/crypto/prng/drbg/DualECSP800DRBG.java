...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f7902_P;

    /* renamed from: _Q */
    private ECPoint f7903_Q;
    private ECCurve.C3628Fp _curve;
    ...
    /* renamed from: _s */
    private byte[] f7904_s;
    private int _sLength;
    private int _securityStrength;
    private int _seedlen;

    static {
        ECCurve.C3628Fp fp = (ECCurve.C3628Fp) NISTNamedCurves.getByName("P-256").getCurve();
        nistPoints[0] = new DualECPoints(128, fp.createPoint(p256_Px, p256_Py), fp.createPoint(p256_Qx, p256_Qy), 1);
        ECCurve.C3628Fp fp2 = (ECCurve.C3628Fp) NISTNamedCurves.getByName("P-384").getCurve();
        nistPoints[1] = new DualECPoints(192, fp2.createPoint(p384_Px, p384_Py), fp2.createPoint(p384_Qx, p384_Qy), 1);
        ECCurve.C3628Fp fp3 = (ECCurve.C3628Fp) NISTNamedCurves.getByName("P-521").getCurve();
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            while (true) {
                if (i2 == dualECPointsArr.length) {
                    ...
                    this.f7902_P = dualECPointsArr[i2].getP();
                    this.f7903_Q = dualECPointsArr[i2].getQ();
                } else {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                }
            }
            if (this.f7902_P != null) {
                this.f7904_s = Utils.hash_df(this._digest, concatenate, this._seedlen);
                this._sLength = this.f7904_s.length;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr2 != null) {
                bigInteger = new BigInteger(1, xor(this.f7904_s, Utils.hash_df(this._digest, bArr2, this._seedlen)));
            } else {
                bigInteger = new BigInteger(1, this.f7904_s);
            }
            int i = 0;
            ...
            for (int i3 = 0; i3 < length2; i3++) {
                bigInteger2 = getScalarMultipleXCoord(this.f7902_P, bigInteger2);
                byte[] byteArray = getScalarMultipleXCoord(this.f7903_Q, bigInteger2).toByteArray();
            ...
            if (i2 < bArr.length) {
                bigInteger2 = getScalarMultipleXCoord(this.f7902_P, bigInteger2);
                byte[] byteArray2 = getScalarMultipleXCoord(this.f7903_Q, bigInteger2).toByteArray();
            ...
            this.f7904_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f7902_P, bigInteger2));
            return length;
        }
    }
    ...
    public void reseed(byte[] bArr) {
        if (!Utils.isTooLarge(bArr, 512)) {
            this.f7904_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f7904_s, this._seedlen), getEntropy(), bArr), this._seedlen);
            this._reseedCounter = 0;
            return;
        }
