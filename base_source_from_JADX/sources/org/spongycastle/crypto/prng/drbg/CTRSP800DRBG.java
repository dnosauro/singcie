...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private byte[] Block_Cipher_df(byte[] bArr, int i) {
        ...
        byte[] bArr3 = new byte[((i3 / 8) + blockSize)];
        byte[] bArr4 = new byte[blockSize];
        byte[] bArr5 = new byte[blockSize];
        byte[] bArr6 = new byte[(i3 / 8)];
    ...
    private void CTR_DRBG_Instantiate_algorithm(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        ...
        this.f7899_V = new byte[blockSize];
        CTR_DRBG_Update(Block_Cipher_df, this._Key, this.f7899_V);
        this._reseedCounter = 1;
    }

    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f7899_V);
        this._reseedCounter = 1;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArr3, this._Key, this.f7899_V);
        } else {
            bArr3 = new byte[this._seedLength];
        }
        byte[] bArr4 = new byte[this.f7899_V.length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i <= bArr.length / bArr4.length; i++) {
            int length = bArr.length - (bArr4.length * i) > bArr4.length ? bArr4.length : bArr.length - (this.f7899_V.length * i);
            if (length != 0) {
                addOneTo(this.f7899_V);
                this._engine.processBlock(this.f7899_V, 0, bArr4, 0);
