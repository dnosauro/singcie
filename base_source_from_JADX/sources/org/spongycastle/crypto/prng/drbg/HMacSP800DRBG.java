...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _K */
    private byte[] f7905_K;

    /* renamed from: _V */
    private byte[] f7906_V;
    ...
    public HMacSP800DRBG(Mac mac, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        if (i > Utils.getMaxSecurityStrength(mac)) {
            ...
            this.f7905_K = new byte[mac.getMacSize()];
            this.f7906_V = new byte[this.f7905_K.length];
            Arrays.fill(this.f7906_V, (byte) 1);
    ...
    private void hmac_DRBG_Update_Func(byte[] bArr, byte b) {
        this._hMac.init(new KeyParameter(this.f7905_K));
        Mac mac = this._hMac;
        byte[] bArr2 = this.f7906_V;
        mac.update(bArr2, 0, bArr2.length);
        this._hMac.update(b);
        if (bArr != null) {
            this._hMac.update(bArr, 0, bArr.length);
        }
        this._hMac.doFinal(this.f7905_K, 0);
        this._hMac.init(new KeyParameter(this.f7905_K));
        Mac mac2 = this._hMac;
        byte[] bArr3 = this.f7906_V;
        mac2.update(bArr3, 0, bArr3.length);
        this._hMac.doFinal(this.f7906_V, 0);
    }
