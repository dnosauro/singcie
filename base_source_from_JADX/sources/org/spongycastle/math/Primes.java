...
public abstract class Primes {
    ...
    private static STOutput implSTRandomPrime(Digest digest, int i, byte[] bArr) {
        ...
        if (i2 < 33) {
            ...
            do {
                ...
                if (isPrime32(extract32)) {
                    return new STOutput(BigInteger.valueOf(extract32), bArr2, i3);
                }
            } while (i3 <= i2 * 4);
        ...
        BigInteger bit = hashGen(digest2, primeSeed, i5).mod(ONE.shiftLeft(i4)).setBit(i4);
        ...
        while (true) {
            if (add.bitLength() > i2) {
                bigInteger = ONE.shiftLeft(i4).subtract(ONE).divide(shiftLeft).add(ONE).shiftLeft(1);
            ...
            if (!implHasAnySmallFactors(add)) {
                ...
                bigInteger = bigInteger.add(BigInteger.valueOf((long) i6));
