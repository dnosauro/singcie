...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature) {
        if (isAlgIdEqual(this.f7965c.getSignatureAlgorithm(), this.f7965c.getTBSCertificate().getSignature())) {
            X509SignatureUtil.setSignatureParameters(signature, this.f7965c.getSignatureAlgorithm().getParameters());
    ...
    private static java.util.Collection getAlternativeNames(byte[] r5) {
        ...
        L_0x00aa:
            ...
            throw r0
        */
        throw new UnsupportedOperationException("Method not decompiled: org.spongycastle.jce.provider.X509CertificateObject.getAlternativeNames(byte[]):java.util.Collection");
    }

    private byte[] getExtensionBytes(String str) {
        Extension extension;
        Extensions extensions = this.f7965c.getTBSCertificate().getExtensions();
    ...
    public void checkValidity(Date date) {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f7965c.getEndDate().getTime());
        } else if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f7965c.getStartDate().getTime());
        }
    }
    ...
    public byte[] getEncoded() {
        try {
            return this.f7965c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public Principal getIssuerDN() {
        try {
            return new X509Principal(X500Name.getInstance(this.f7965c.getIssuer().getEncoded()));
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f7965c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            ...
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f7965c.getIssuer());
    ...
    public Date getNotAfter() {
        return this.f7965c.getEndDate().getDate();
    }

    public Date getNotBefore() {
        return this.f7965c.getStartDate().getDate();
    }

    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f7965c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f7965c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f7965c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    public byte[] getSigAlgParams() {
        if (this.f7965c.getSignatureAlgorithm().getParameters() != null) {
            try {
                return this.f7965c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
            } catch (IOException unused) {
            }
        }
        return null;
    ...
    public byte[] getSignature() {
        return this.f7965c.getSignature().getOctets();
    }
    ...
    public Principal getSubjectDN() {
        return new X509Principal(X500Name.getInstance(this.f7965c.getSubject().toASN1Primitive()));
    }

    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f7965c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Principal getSubjectX500Principal() {
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f7965c.getSubject());
            return new X500Principal(byteArrayOutputStream.toByteArray());
        } catch (IOException unused) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    ...
    public byte[] getTBSCertificate() {
        try {
            return this.f7965c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public int getVersion() {
        return this.f7965c.getVersionNumber();
    }

    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f7965c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration oids = extensions.oids();
        while (oids.hasMoreElements()) {
    ...
    public final void verify(PublicKey publicKey) {
        Signature signature;
        String signatureName = X509SignatureUtil.getSignatureName(this.f7965c.getSignatureAlgorithm());
        try {
