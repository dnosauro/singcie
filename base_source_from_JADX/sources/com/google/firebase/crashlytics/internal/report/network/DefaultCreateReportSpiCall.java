...
public class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Logger.getLogger().mo21676d("Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return part.part(FILE_PARAM, report.getFileName(), FILE_CONTENT_TYPE, report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Logger.getLogger().mo21676d("Adding file " + file.getName() + " to report " + report.getIdentifier());
    ...
    public boolean invoke(CreateReportRequest createReportRequest, boolean z) {
        if (z) {
            ...
            try {
                ...
                logger2.mo21676d("Create report request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                Logger logger3 = Logger.getLogger();
                logger3.mo21676d("Result was: " + code);
                return ResponseParser.parse(code) == 0;
            } catch (IOException e) {
                Logger.getLogger().mo21679e("Create report HTTP request failed.", e);
