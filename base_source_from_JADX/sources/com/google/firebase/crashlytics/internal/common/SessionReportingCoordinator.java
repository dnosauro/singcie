...
class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            logger.mo21676d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            this.reportPersistence.deleteFinalizedReport(result.getSessionId());
            return true;
        }
        Logger.getLogger().mo21677d("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private void persistEvent(Throwable th, Thread thread, String str, long j, boolean z) {
        ...
        if (str2 == null) {
            Logger.getLogger().mo21676d("Cannot persist event, no currently open session");
            return;
        }
    ...
    public void persistUserId() {
        ...
        if (str == null) {
            Logger.getLogger().mo21676d("Could not persist user ID; no current session");
            return;
        }
        String userId = this.reportMetadata.getUserId();
        if (userId == null) {
            Logger.getLogger().mo21676d("Could not persist user ID; no user ID available");
        } else {
    ...
    public Task<Void> sendReports(Executor executor, DataTransportState dataTransportState) {
        if (dataTransportState == DataTransportState.NONE) {
            Logger.getLogger().mo21676d("Send via DataTransport disabled. Removing DataTransport reports.");
            this.reportPersistence.deleteAllReports();
        ...
        for (CrashlyticsReportWithSessionId next : loadFinalizedReports) {
            if (next.getReport().getType() != CrashlyticsReport.Type.NATIVE || dataTransportState == DataTransportState.ALL) {
                ...
                Logger.getLogger().mo21676d("Send native reports via DataTransport disabled. Removing DataTransport reports.");
