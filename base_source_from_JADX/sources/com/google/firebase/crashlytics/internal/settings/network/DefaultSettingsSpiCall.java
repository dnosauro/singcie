...
public class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            logger2.mo21677d("Failed to parse settings JSON from " + getUrl(), e);
            Logger logger3 = this.logger;
            logger3.mo21676d("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger2 = this.logger;
        logger2.mo21676d("Settings result was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger3 = this.logger;
        logger3.mo21678e("Failed to retrieve settings from " + getUrl());
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                Logger logger2 = this.logger;
                logger2.mo21676d("Requesting settings from " + getUrl());
                Logger logger3 = this.logger;
                logger3.mo21676d("Settings query params were: " + queryParamsFor);
                HttpResponse execute = applyHeadersTo.execute();
                Logger logger4 = this.logger;
                logger4.mo21676d("Settings request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                return handleResponse(execute);
            } catch (IOException e) {
                this.logger.mo21679e("Settings request failed.", e);
                return null;
            }
        } else {
