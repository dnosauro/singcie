...
public class CrashlyticsReportPersistence {
    ...
    private static void synthesizeNativeReportFile(File file, File file2, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            logger.mo21677d("Could not synthesize final native report file for " + file, e);
        }
    }
        ...
        if (!filesInDirectory.isEmpty()) {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        Logger logger = Logger.getLogger();
                        logger.mo21677d("Could not add event to report for " + next, e);
                    }
                }
            }
            String str = null;
            ...
            if (file2.isFile()) {
                try {
                    ...
                    logger2.mo21677d("Could not read user ID file in " + file.getName(), e2);
                }
            }
    ...
    private static void synthesizeReportFile(File file, File file2, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo21677d("Could not synthesize final report file for " + file, e);
        }
    }
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File next : getAllFinalizedReportFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.mo21677d("Could not load report file " + next + "; deleting", e);
                next.delete();
            }
        }
        return arrayList;
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo21677d("Could not persist event for session " + str, e);
        }
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().mo21676d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo21677d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistUserIdForSession(String str, String str2) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo21677d("Could not persist user ID for session " + str2, e);
        }
    }
}
