    ...
    public Task<Void> continueWhile(Callable<Boolean> callable, Continuation<Void, Task<Void>> continuation, Executor executor, CancellationToken cancellationToken) {
        ...
        capture.set(new Continuation<Void, Task<Void>>() {
            public Task<Void> then(Task<Void> task) {
                ...
                return (cancellationToken == null || !cancellationToken.isCancellationRequested()) ? ((Boolean) callable2.call()).booleanValue() ? Task.forResult(null).onSuccessTask(continuation2, executor2).onSuccessTask((Continuation) capture2.get(), executor2) : Task.forResult(null) : Task.cancelled();
            }
        });
